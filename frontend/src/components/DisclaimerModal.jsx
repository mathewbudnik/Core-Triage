      ...
      >
        ...
        <div className="bg-ct-terracotta/10 border-b border-ct-terracotta/20 px-6 py-5 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-ct-cream/60 mt-0.5">Please read before continuing</p>
          </div>
        </div>
        ...
        <div className="px-6 py-5 space-y-5 overflow-y-auto flex-1">
          ...
          {/* Agreement points */}
          <div>
            <p className="text-xs font-semibold text-ct-cream/60 uppercase tracking-wide mb-3">
              By continuing you acknowledge and agree that:
            </p>
            <ul className="space-y-2">
              {POINTS.map((point, i) => (
                <li key={i} className="flex gap-2 text-xs text-ct-cream/60 leading-relaxed">
          ...
          {!readOnly && (
            <div className="border-t border-ct-hairline pt-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-xs font-semibold text-ct-cream/60 uppercase tracking-wide">
        ...
        <div className="border-t border-ct-hairline px-6 py-4">
          {readOnly ? (
            <button
              onClick={onExit}
              className="w-full px-4 py-2 rounded-lg text-sm font-medium bg-ct-hairline text-ct-cream/80 border border-ct-rim hover:brightness-110 transition-all duration-200"
            >
              Close
            </button>
          ) : (
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={onExit}
                className="flex-1 px-4 py-2.5 rounded-lg text-sm font-medium bg-ct-hairline text-ct-cream/80 border border-ct-rim hover:brightness-110 transition-all duration-200 order-2 sm:order-1"
              >
                Exit App
              </button>
              <button
