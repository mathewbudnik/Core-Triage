...
import { Send, Loader2, Bot, User, Cpu, Sparkles, Radio } from 'lucide-react'
import ReactMarkdown from 'react-markdown'
import { sendChat, getOllamaStatus } from '../api'
...
const MODE_META = {
  ...
  ollama: { label: 'Ollama', icon: Radio, color: 'text-accent2', desc: 'Local LLM via Ollama' },
}

export default function ChatTab({ k }) {
  ...
  const availableModes = ollamaAvailable
    ? ['kb', 'gpt', 'ollama']
    : ['kb', 'gpt']
      ...
      <div className="border-b border-outline px-6 py-3 flex items-center gap-4 flex-wrap bg-panel2/40">
        ...
        {mode === 'ollama' && (
          <input
            ...
            className="input-base w-48 text-xs py-1.5"
          />
        )}

        {!ollamaAvailable && (
          <span className="text-xs text-muted">Ollama not detected locally</span>
        )}

        <div className={`ml-auto text-xs font-medium flex items-center gap-1.5 ${MODE_META[mode].color}`}>
          {(() => { const Icon = MODE_META[mode].icon; return <Icon size={12} /> })()}
          {MODE_META[mode].desc}
        </div>
      </div>
