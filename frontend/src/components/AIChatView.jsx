...
const MessageItem = memo(function MessageItem({ msg }) {
  return (
    ...
    >
      {msg.role === 'assistant' && (
        <div className="w-7 h-7 rounded-lg bg-clay/10 border border-clay/30 flex items-center justify-center shrink-0 mt-1">
          <Bot size={14} className="text-clay-deep" />
        </div>
      )}
      ...
      {msg.role === 'user' && (
        <div className="w-7 h-7 rounded-lg bg-clay/15 border border-clay/25 flex items-center justify-center shrink-0 mt-1">
          <User size={14} className="text-clay-deep" />
        </div>
      )}
    </motion.div>
  )
...
export default function AIChatView({ k, user, onBack }) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="border-b border-ct-hairline px-4 md:px-6 py-3 flex items-center gap-3 flex-wrap bg-[linear-gradient(180deg,rgba(253,246,234,0.85),rgba(244,236,219,0.4))]">
        <button
          onClick={onBack}
        ...
        <div className="ml-auto text-[11px] font-medium flex items-center gap-1.5">
          {chatUnlimited ? (
            <span className="text-clay-deep">Unlimited</span>
          ) : (
            <span className={chatUsed >= FREE_CHAT_LIMIT - 1 ? 'text-clay-deep font-semibold' : 'text-ink-soft'}>
        ...
        <div className="px-4 md:px-6 pt-3">
          <div
            className="inline-flex items-center gap-1.5 text-[11px] font-semibold
                       bg-clay/10 border border-clay/25
                       rounded-full px-2.5 py-1 text-clay-deep"
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-6 py-6 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center py-16 space-y-4">
            <div className="w-16 h-16 rounded-2xl bg-clay/10 border border-clay/30 flex items-center justify-center">
              <Bot size={28} className="text-clay-deep" />
            </div>
            <div>
              <p className="text-ink font-serif font-semibold text-lg">CoreTriage Assistant</p>
            ...
            <div className="flex flex-wrap gap-2 justify-center max-w-md">
              {[
                ...
                <button
                  key={q}
                  onClick={() => setInput(q)}
                  className="text-xs ct-surface rounded-full px-3 py-1.5 text-ink-soft hover:text-clay-deep hover:border-clay/40 transition-colors"
                >
                  {q}
                </button>
              ))}
        ...
        {loading && (
          ...
          >
            <div className="w-7 h-7 rounded-lg bg-clay/10 border border-clay/30 flex items-center justify-center shrink-0">
              <Bot size={14} className="text-clay-deep" />
            </div>
            <div className="ct-surface rounded-2xl rounded-tl-sm px-4 py-3 shadow-[0_2px_8px_rgba(42,39,34,0.08)]">
      ...
      <div className="px-4 md:px-6 pt-3 pb-0">
        <div className="flex items-center gap-2 bg-clay/10 border border-clay/25 rounded-lg px-3 py-2">
          <AlertTriangle size={12} className="text-clay-deep shrink-0" />
          <p className="text-[11px] text-ink-soft">
            General guidance only — not medical advice. Emergencies: call <strong className="text-ink">911</strong>.
          </p>
        </div>
      </div>

      {/* Input */}
      <div className="border-t border-ct-hairline p-4 bg-[linear-gradient(0deg,rgba(253,246,234,0.85),rgba(244,236,219,0.4))] mt-3">
        {/* Free-limit banner — only when approaching/at limit */}
        {!chatUnlimited && chatUsed >= FREE_CHAT_LIMIT - 1 && (
          <div className="mb-3 flex items-center justify-between ct-surface rounded-xl px-3 py-2.5 gap-3">
            <div className="flex items-center gap-2">
              <Lock size={12} className="text-clay-deep shrink-0" />
              {limitExceeded ? (
        ...
        <div className="flex items-center justify-between mt-1.5">
          ...
          <span className={`text-[11px] shrink-0 ml-3 tabular-nums ${overLimit ? 'text-clay-deep font-semibold' : 'text-ink-muted'}`}>
            {charCount}/{MAX_CHARS}
          </span>
        </div>
        {overLimit && (
          <p className="text-xs text-clay-deep mt-1">Message too long — please shorten it before sending.</p>
        )}
      </div>
